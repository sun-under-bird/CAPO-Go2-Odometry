...
namespace
{
  ...
  // ==== CAPO 仿真适配（新增）：桥接对象的全局原子指针。
  // UnitreeSdk2BridgeThread 创建 bridge 后 store，物理线程每步 mj_step 后
  // load 并调用 ComputeFootForce() 写入足端接触力缓存（线程分工，避免桥接线程
  // 与物理线程并发遍历 mjData->contact 引发的撕裂读段错误）。====
  std::atomic<UnitreeSDK2BridgeBase *> g_bridge{nullptr};
...

      {
        ...
        if (m)
        {
          ...
          if (sim.run)
          {
            ...
            // ==== CAPO 仿真适配（新增）：物理线程计算足端接触力缓存。
            // 此时 mjData 状态一致（mj_step 刚返回），无并发撕裂读风险。====
            if (stepped)
            {
              if (auto *bridge = g_bridge.load(std::memory_order_acquire))
              {
                bridge->ComputeFootForce();
              }
            }
...
void *UnitreeSdk2BridgeThread(void *arg)
{
  ...
  interface->start();
  g_bridge.store(interface.get()); // 注册给物理线程，供 mj_step 后计算足端接触力
